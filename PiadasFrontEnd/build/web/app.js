...
app.controller("piadactrl", function($scope,$http){
    ...
    $scope.listar = function(){
        $scope.buscando = true;
        $http.get("http://piada.herokuapp.com/piada").then(function(result){
    ...
    $scope.salvar = function(piada){
        $scope.enviando=true;
        console.log(piada);
        $http.post("http://piada.herokuapp.com/piada",piada).then(function(result){
            $scope.enviando=false;
            $scope.mensagem="Piada cadastrada com sucesso!";
            console.log(result);
        }, function(erro){
            $scope.enviando=false;
            $scope.menagem="Erro no cadastro.";
            console.log(erro);
        }); 
